...
def parse_args():
	...
	parser.add_argument('--root_index', type=int, default=9, help='root index for normalization')
...
def read_img(idx, imgdir, fixed_str, image_version=None):

	image_rgb_path = os.path.join(imgdir, fixed_str, 'rgb', '%08d.jpg' % sample_version.map_id(idx, image_version))

	_assert_exist(image_rgb_path)
	return cv2.imread(image_rgb_path, cv2.IMREAD_COLOR)
...
def train_gen(train_idxs, args):
	train_xyz_labels_mat = np.zeros((train_idxs.shape[0], args.num_keypoints, 3))
	for train_idx in train_idxs:
		...
		### Retrieve the labels ###
		train_xyz_labels_mat[train_idx,:,:] = norm_lite.start(train_xyz, 1.0, np.eye(3),
															  train_K, 1.0, False, args.num_keypoints, args.root_index)
		train_xyz_mat[train_idx,:,:] = train_xyz
		print("train idx: ", train_idx)
	train_xyz_labels_mat = train_xyz_labels_mat - train_xyz_labels_mat.mean(1, keepdims=True)
	np.save(os.path.join(args.localdir, 'hand_train.npy'), train_xyz_labels_mat)	
	print("------------------ Train data is split and ready to use. -----------------")

def test_gen(test_idxs, args):
	test_xyz_labels_mat = np.zeros((test_idxs.shape[0], args.num_keypoints, 3))
	counter = 0
	for test_idx in test_idxs:
		...
		### Retrieve the labels ###
		test_xyz_labels_mat[counter,:,:] = norm_lite.start(test_xyz, 1.0, np.eye(3),
															test_K, 1.0, False, args.num_keypoints, args.root_index)
		counter = counter + 1
		print("test idx: ", test_idx)

	test_xyz_labels_mat = test_xyz_labels_mat - test_xyz_labels_mat.mean(1, keepdims=True)
	np.save(os.path.join(args.localdir, 'hand_test.npy'), test_xyz_labels_mat)	
